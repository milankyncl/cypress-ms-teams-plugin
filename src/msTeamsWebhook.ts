...
export function getResultMessagePayload(options: Pick<Options, 'ref'>, result: RunResult) {
    const failedCases = result.runs.map((run) => {
        if (run.stats.failures > 0) {
            return run.tests.map((test) => test.state === 'failed'
                ? `${test.title.join(' : ')} (${run.spec.name})` : null).filter((test): test is string => test !== null);
        }
        return [];
    }).flat();
    ...
    return {
        ...
        title: failed ? `${result.totalFailed} test cases failed` : 'All test cases passed',
        sections: ([
            options.ref ? {
                activityTitle: options.ref,
                text: `Test run completed finished in ${(result.totalDuration / 1000).toString()}s`,
            } : undefined,
            failed ? {
                text: failed ? ['Failed test cases:', ...failedCases].join('\n') : undefined,
            } : undefined
